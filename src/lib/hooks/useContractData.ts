...
export function useContractData() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.log('Scopes cursor:', scopesCursor);
      ...
      console.log('Raw table data:', { poolsData, tiersData, configData });

      console.log('Fetching stakes for each scope...');
      const allStakesPromises = scopes.map(async (scope: TableScope) => {
        try {
          console.log('Fetching stakes for scope:', scope.scope);
          ...
          console.log('Stakes for scope', scope.scope, ':', userStakes);
          ...
          console.error('Error fetching stakes for scope', scope.scope, ':', scopeError);
          return [];
        }
      });

      console.log('Waiting for all stakes promises...');
      ...
      console.log('Transforming pools data...');
      const pools: PoolEntity[] = poolsData.map(pool => {
        try {
          ...
          console.error('Error transforming pool:', pool, poolError);
          throw poolError;
        }
      });

      // Transform tiers data
      console.log('Transforming tiers data...');
      const tiers: TierEntity[] = tiersData.map(tier => {
        try {
          ...
          console.error('Error transforming tier:', tier, tierError);
          throw tierError;
        }
      });

      // Transform config data
      console.log('Transforming config data...');
