...
const GameUI: React.FC = () => {
  ...
  // Effect to fetch initial data when session changes
  useEffect(() => {
    if (session) {
      refreshData();
    } else {
      // Clear data when session ends
      setPools([]);
  ...
  // Effect to refresh data when pool is selected
  useEffect(() => {
    if (selectedPool && session) {
      console.log('Selected pool changed, refreshing data...', selectedPool);
      refreshData();
    }
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        // If no pool is selected and we have pools, select the first one
...
// Calculate tier progress only when all required data is available
  const tierProgress = React.useMemo(() => {
    if (!playerStake || !selectedPool || !tiers.length) {
      console.log('Missing data for tier progress:', { playerStake, selectedPool, tiersLength: tiers.length });
      return null;
    }
    ...
    try {
      ...
      console.log('Calculated tier progress:', progress);
      return progress;
    } catch (error) {
  ...
  const canUpgradeTier = React.useMemo(() => {
    if (!tierProgress?.currentTier || !selectedPool || !playerStake) {
      console.log('Missing data for upgrade check:', { tierProgress, selectedPool, playerStake });
      return false;
    }

    try {
      const canUpgrade = isTierUpgradeAvailable(
        playerStake.staked_quantity,
        selectedPool.total_staked_quantity,
        tierProgress.currentTier,
        tiers
      );
      console.log('Can upgrade tier:', canUpgrade);
      return canUpgrade;
    } catch (error) {
